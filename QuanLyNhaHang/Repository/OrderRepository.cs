...
namespace QuanLyNhaHang.Repository
{
    public class OrderRepository : GenericRepository<Order>
    {
        ...
        public async Task<(IEnumerable<Order> lstOrder, int TotalCount)> GetAll(GetAllResquest request, bool import, string code)
        {
            
            var queryBuilder = new StringBuilder("SELECT * FROM [Order] WHERE UserLoginCode like '%" + code + "%'");
            ...
            if (request.Search is not null && request.Search.Length > 0)
            {
                ...
                queryBuilder.Append($" UserLoginId LIKE N'%{request.Search}%'");
            }
        ...
        public async Task CreateOrderAsync(Order request)
        {
            
            Product model = productRepo.GetByID(request.ProductId);
            if(model != null)
            {
                model.Stock = model.Stock - request.Qty;
                productRepo.Update(model); // update stock
                Create(request);
            }
        }
