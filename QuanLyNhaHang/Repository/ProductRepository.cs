...
namespace QuanLyNhaHang.Repository
{
    ...
    {
        public async Task<(IEnumerable<Product> lstProduct, int TotalCount)> GetAll(GetAllResquest request, bool import)
        {
            var queryBuilder = new StringBuilder("SELECT * FROM Product");
            ...
            if (request.Search is not null && request.Search.Length > 0)
            {
                ...
                queryBuilder.Append($" Name LIKE N'%{request.Search}%'");
            }
            string countQuery = queryBuilder.ToString().Replace("SELECT * FROM", "SELECT COUNT(*) FROM");
            int totalCount = await SQLHelper<Product>.SqlToScalar<int>(countQuery, SQL_QLNH);
            ...
            var data = await SQLHelper<Product>.SqlToList(queryBuilder.ToString(), SQL_QLNH);
